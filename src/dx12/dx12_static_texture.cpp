...
namespace feng
{
    StaticTexture::StaticTexture(Device& device, DirectX::ResourceUploadBatch& uploader, const std::wstring& path)
    {
        ...
        DirectX::CreateShaderResourceView(device.GetDevice(), buffer_.Get(), device.GetSTSRVHeap().GetCpuHandle(device.GetSTSRVAllocIndex()));
    }
    ...
    void StaticMaterial::Init(Device &device, DirectX::ResourceUploadBatch &uploader)
    {
        ...
        //     textures_.emplace(normal_path_, normal_path_);
        // }
