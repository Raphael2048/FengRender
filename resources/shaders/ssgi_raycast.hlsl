...
Texture2D t_reduction : register(t5);

RWTexture2D<float4> t_ssgi : register(u0);
...
float3 RacyCast(float3 N, float3 V, float3 R)
{
...
{
    t_ssgi[DispatchThreadId] = float4(0, 0, 0, 0);
    ...
    while((t < tMax) && (IterCount < 48))
    {
        ...
        if (ViewSpaceSampleZ > RayZ)
        {
            ...
            {
                // 此时视为成功找到
                float3 color = t_reduction.SampleLevel(linear_sampler, RayPos * InvScreenSize, RoughnessMetallic.x * 3).rgb;
                ...
                float3 kD = (1.0 - F) * (1.0 - RoughnessMetallic.y);
                t_ssgi[DispatchThreadId] = float4(color * kD, 0);
                break;
            }
            else
            {
